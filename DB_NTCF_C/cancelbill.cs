    ...
    public partial class cancelbill : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Close();
                    cmd = new MySqlCommand("SELECT * FROM recretailer WHERE recRetailer_ID = '" + textBox1.Text + "'", con);
                    con.Open();
                    read = cmd.ExecuteReader();
                    if (read.Read())
                    {
                        con.Close();
                        cmd = new MySqlCommand("INSERT INTO copy (Qty,Product_ID) SELECT Qty,Product_ID FROM recdetailretailer WHERE recRetailer_ID ="+ textBox1.Text + " ;DELETE FROM recretailer WHERE recRetailer_ID = '" + textBox1.Text + "'", con);
